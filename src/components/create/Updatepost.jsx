...
const Updatepost = () => {
    ...
    const [value, setValue] = useState(postEntries.companyName)
    const [inputValue, setInputValue] = useState(postEntries.companyName)
    ...
    const updateBlogPost = async() => {
        let response = await API.updatePost(postEntries);
        if(response.isSuccess){
            navigate(`/details/${id}`)
        }
    }
  ...
  return (
    ...
    <InputBox value={postEntries.role} onChange={(e)=>{handleChange(e)}} name='role' placeholder='Enter Position/Role'></InputBox>

    <LabelOfInput>Eligibility Criteria:</LabelOfInput>
    <InputBox value={postEntries.eligibility} onChange={(e)=>{handleChange(e)}} name='eligibility' placeholder='Eligbility'></InputBox>

    <LabelOfInput>Format of Interview:</LabelOfInput>
    <InputBox value={postEntries.format} onChange={(e)=>{handleChange(e)}} name='format' placeholder='Enter Format'></InputBox>

    <LabelOfInput>Detailed Description:</LabelOfInput>
    <TextArea value={postEntries.description} onChange={(e)=>{handleChange(e)}} name='description' minRows={5} placeholder='Write a Description'></TextArea>

    <LabelOfInput>Suggestions:</LabelOfInput>
    <TextArea value={postEntries.suggestions} onChange={(e)=>{handleChange(e)}} name='suggestions' minRows={3} placeholder='Any Suggestions'></TextArea>
